...
function extractTextContent(nodes: PONode[]): string {
  ...
  const processNode = (node: PONode): void => {
    ...
    // TODO: how to proceed with formatting in text
    switch (tagName) {
      case 'b':
        result += '**';
...
function extractValign(node: PONode): VerticalAlign {
  const attrs = attrsOf(node);
  return attrs.valign || 'top';
}
...
function parseTable(tableNode: PONode, parentId?: string): TableNode {
  ...
  if (theadEl) {
    ...
    const valign = extractValign(theadEl);

    head = {
      rows: theadRows.map((rowNode, rowNo) =>
        parseTableRow(rowNode, columnNames, valign, buildId(tableId, 'h', rowNo))
      ),
    };
  }
  ...
  if (tbodyEl) {
    ...
    const valign = extractValign(tbodyEl);
    body = {
      rows: tbodyRows.map((rowNode, rowNo) =>
        parseTableRow(rowNode, columnNames, valign, buildId(tableId, 'b', rowNo))
      ),
    };
  }
...
function parseTableRow(
  ...
  const cells = entries.map((entryNode, colNo) =>
    parseTableCell(entryNode, columnNames, buildId(parentId, colNo))
  );
  return {
    valign: valign,
    cells,
        ...
        for (const child of contentChildren) {
          ...
          if (tagName === 'p') {
            ...
            const block: BlockNode = {
              type: 'block',
              id: blockId,
              children: parseContentNodes(childrenOf(child), blockId),
            };
